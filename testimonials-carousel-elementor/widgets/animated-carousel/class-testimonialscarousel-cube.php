...
class TestimonialsCarousel_Cube extends Widget_Base
{
  ...
  {
    return \TestimonialsCarouselElementor\Testimonials_Carousel_Assets::get_widget_script_depends(array('swiper', 'ai-btn'));
  }
  ...
  protected function register_controls()
  {
    ...
    $this->add_responsive_control(
        ...
        [
            ...
            'selectors'  => [
                '{{WRAPPER}} .mySwiperCube' => '--container-widget-max-width: {{SIZE}}{{UNIT}}; --tc-cube-size: {{SIZE}}{{UNIT}}; --container-widget-flex-grow: 0; width: min(100%, {{SIZE}}{{UNIT}}); max-width: min(100%, {{SIZE}}{{UNIT}}); height: {{SIZE}}{{UNIT}}; margin-bottom: calc({{SIZE}}{{UNIT}} * 0.12); overflow: visible;',
            ],
        ]
    );
    $this->add_responsive_control(
        ...
        [
            ...
            'selectors'  => [
                '{{WRAPPER}} .mySwiperCube' => '--tc-cube-size: {{SIZE}}{{UNIT}}; width: {{SIZE}}{{UNIT}}; max-width: min(100%, {{SIZE}}{{UNIT}}); height: {{SIZE}}{{UNIT}}; margin-bottom: calc({{SIZE}}{{UNIT}} * 0.12); aspect-ratio: unset; overflow: visible;',
            ],
        ]
    );
  ...
  protected function render()
  {
    ...
    if ($settings['slide']) {
      ...
      <section
          class="swiper mySwiper mySwiperCube"<?php \TestimonialsCarouselElementor\Testimonials_Carousel_Slider_Render::print_cube_data_attributes($settings); ?>>
            ...
            <li class="swiper-slide slider-container-background">
              <?php if (isset($item['slide_image'], $item['slide_image']['url']) && !empty($item['slide_image']['url'])) { ?>
                <img
                    ...
                    alt="<?php echo esc_html($item['slide_name']); ?>">
              <?php }

              if ($item['slide_price_enable'] === 'yes') { ?>
                <div class="cost"><?php echo wp_kses($item['slide_price'], []); ?></div>
              <?php }

              if ($item['slide_overlay_enable'] === 'yes' && $settings['overlay_enable'] === 'yes') { ?>
                <div class="overlay <?php echo esc_attr('overlay-' . $counter); ?>">
                  <h1><?php echo esc_html($item['slide_name']); ?></h1>
